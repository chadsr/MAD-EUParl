...
if json_str is not None:
    dict_mep = io.json_to_defaultdict(json_str)

json_str = io.load_json(c.DICT_PARTIES)
if json_str is not None:
    dict_party = io.json_to_defaultdict(json_str)
    ...
    for mep in islice(json_data, 0, c.MEP_LIMIT):
        ...
        if 'Birth' in mep:
            ...
            if 'place' in mep['Birth']:
                birth_place = URIRef(name_to_dbr(str(mep['Birth']['place'].strip())))
        ...
        if 'Groups' in mep:
            ...
            party_uri = URIRef(str(dict_party[party_id][0]))
